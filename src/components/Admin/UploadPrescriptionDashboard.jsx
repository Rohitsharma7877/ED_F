...
const UploadPrescriptionDashboard = () => {
  ...
  const fetchPrescriptions = async () => {
    try {
      const response = await fetch("https://ed-b-1.onrender.com/api/prescriptions");
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const response = await fetch(
        `https://ed-b-1.onrender.com/api/prescriptions/${id}/status`,
        {
          method: "PUT",
          headers: {
          ...
          {sortedPrescriptions.map((prescription, index) => (
            <tr key={prescription._id || index}>
              ...
              <td>
                {prescription.file ? (
                  <a
                    href={`https://ed-b-1.onrender.com/uploads/${prescription.file}`}
                    target="_blank"
